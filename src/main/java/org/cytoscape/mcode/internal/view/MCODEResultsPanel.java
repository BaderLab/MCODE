...
public class MCODEResultsPanel extends JPanel implements CytoPanelComponent {
	 ...
	 * 
	 * @param clusters
	 *            Found clusters from the MCODEAnalyzeTask
	 * @param alg
	...
	public MCODEResultsPanel(MCODECluster[] clusters,
		...
		// the TableRowSelectionHandler below)
		//		networkView = Cytoscape.getNetworkView(network.getSUID());
	...
	public double setNodeAttributesAndGetMaxScore() {
		...
		//						clusterArrayList = (ArrayList) Cytoscape.getNodeAttributes()
		//								.getAttributeList(n.getIdentifier(), "MCODE_Cluster");
		//						clusterArrayList.add(cluster.getClusterName());
		//					} else {
		//						clusterArrayList.add(cluster.getClusterName());
		//					}
		//					setAttributeList(Cytoscape.getNodeAttributes(),
		//									 n.getIdentifier(),
		//									 "MCODE_Cluster",
		//									 clusterArrayList);
		//
		...
		//													   "MCODE_Score",
		//													   alg.getNodeScore(n.getRootGraphIndex(), resultId));
		//		}
	...
	//			final String title = trigger.getResultTitle() + ": " + cluster.getClusterName() + " (Score: " +
	...
	//					CyNetwork newNetwork = Cytoscape.createNetwork(gpCluster.getNodeIndicesArray(), gpCluster
	...
	//							cluster.getView().getNodeView(nv.getNode().getRootGraphIndex()) != null) {
	//							// If it does, then we take the layout position that
	//							// was already generated for it
	//							nv.setXPosition(cluster.getView().getNodeView(nv.getNode().getRootGraphIndex())
	//									.getXPosition());
	//							nv.setYPosition(cluster.getView().getNodeView(nv.getNode().getRootGraphIndex())
	//									.getYPosition());
	//						} else {
	...
	//
	//		public MCODEClusterBrowserTableModel(ImageName imageList[]) {
	...
	//						attributeValue.add(Cytoscape.getNodeAttributes().getStringAttribute(node.getIdentifier(),
	//																							attributeName));
	//					} else if (Cytoscape.getNodeAttributes().getType(attributeName) == CyAttributes.TYPE_FLOATING) {
	//						attributeValue.add(Cytoscape.getNodeAttributes().getDoubleAttribute(node.getIdentifier(),
	//																							attributeName));
	//					} else if (Cytoscape.getNodeAttributes().getType(attributeName) == CyAttributes.TYPE_INTEGER) {
	//						attributeValue.add(Cytoscape.getNodeAttributes().getIntegerAttribute(node.getIdentifier(),
	//																							 attributeName));
	//					} else if (Cytoscape.getNodeAttributes().getType(attributeName) == CyAttributes.TYPE_BOOLEAN) {
	//						attributeValue.add(Cytoscape.getNodeAttributes().getBooleanAttribute(node.getIdentifier(),
	//																							 attributeName));
	//					} else if (Cytoscape.getNodeAttributes().getType(attributeName) == CyAttributes.TYPE_SIMPLE_LIST) {
	//						List valueList = Cytoscape.getNodeAttributes().getAttributeList(node.getIdentifier(),
	//																						attributeName);
	...
	//						Map valueMap = Cytoscape.getNodeAttributes().getAttributeMap(node.getIdentifier(),
	//																					 attributeName);
	...
	//			// get the file name
	//			File file = FileUtil.getFile("Export Graph as Interactions", FileUtil.SAVE, new CyFileFilter[] {});
	//
	//			if (file != null) {
	...
	//						ImageName image = MCODEUtil.convertNetworkToImage(loader,
	//																	  cluster,
	//																	  graphPicSize,
	//																	  graphPicSize,
	//																	  layouter,
	...
	//						// statement)
	//						ImageName image = MCODEUtil.getPlaceHolderImage(graphPicSize, graphPicSize);
	//						// Update the table
	//						table.setValueAt(new ImageIcon(image), cluster.getRank(), 0);
	//						// select the cluster
	//						selectCluster(cluster.getGPCluster());
